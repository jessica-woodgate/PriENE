...
class MPDQNAgent():
    ...
    def step(self):
        ...
        if self.done == False:
            ...
            best_action, selected_actions = self._apply_utility(observation,self.epsilon)
            self.current_reward, next_state, self.done = self.interaction_module.perform_transition(best_action)
            if self.n_rewards == 1:
    ...
    def _init_networks(self, checkpoint_path):
        ...
        for objective in self.objectives:
            ...
            target_checkpoint_path = checkpoint_path+self.agent_type+"/agent_"+str(self.unique_id)+"/target_model_variables.keras"
            ...
            q_network = DQN(self.actions,(self.n_features,),self.training,checkpoint_path=self.q_checkpoint_path,shared_replay_buffer=self.shared_replay_buffer)
            target_network = DQN(self.actions,(self.n_features,),self.training,checkpoint_path=self.target_checkpoint_path,shared_replay_buffer=self.shared_replay_buffer)
            if self.training:
    ...
    def _apply_utility(self, observation):
        ...
        for objective in self.networks:
            ...
            actions[objective] = objective["q_network"].choose_action(observation,objective["epsilon"])
        most_common = Counter(actions.values()).most_common(1)
